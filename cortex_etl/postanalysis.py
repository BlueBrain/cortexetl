...
def bursting_test(spiking_hist, features_for_sim):

    min_val = np.min(spiking_hist)
    max_val = np.max(spiking_hist)

    bursting = False
    nc_frs = [features_for_sim.etl.q(neuron_class=nc)['mean_of_mean_firing_rates_per_second'] for nc in c_etl.LAYER_EI_NEURON_CLASSES]
...
def euclidean_distance_to_scaled_in_vivo_FRs(spont_features_for_sim, vivo_frs, target_proportion):
...
def custom_post_analysis_single_simulation(simulation_row, 
    ...
    r_dict['std_of_neuron_class_proportions_of_vivo'], r_dict['cv_of_neuron_class_proportions_of_vivo'], proportions_of_vivo = proportion_of_in_vivo_stats(spont_features_for_sim, analysis_config['vivo_frs'])
    ...
    r_dict['euc_dist_to_scaled_in_vivo_FRs'] = euclidean_distance_to_scaled_in_vivo_FRs(spont_features_for_sim, analysis_config['vivo_frs'], target_proportion)

    r_dict['difference_between_mean_proportion_and_target_proportion'] = target_proportion - np.mean(proportions_of_vivo)

    r_dict['atleast_one_neuron_class_fr_greater_than_invivo_thresh'] = atleast_one_neuron_group_above_threshold_invivo_proportion_test(spont_features_for_sim, analysis_config['vivo_frs'], 1.05)
    r_dict['bursting'], r_dict['bursting_ratio'] = bursting_test(spont_hist_ALL, spont_features_for_sim)
...
def post_analysis(a):
    ...
    ######################################################################
    a.custom['custom_features_by_neuron_class'] = a.features.by_neuron_class.df.reset_index().etl.q(neuron_class=c_etl.LAYER_EI_NEURON_CLASSES).copy()
    ...
    if ('desired_unconnected_fr' in [x for xs in a.analysis_config.custom['fr_comparison_pairs'] for x in xs]):
        a.custom['custom_features_by_neuron_class']['desired_unconnected_fr'] = a.custom['custom_features_by_neuron_class'].apply(lambda row: a.repo.simulations.df.etl.q(simulation_id=row.simulation_id).iloc[0].simulation.__dict__['config'].Run[a.analysis_config.custom['desired_unconnected_fr_key'] + '_' + c_etl.bluepy_neuron_class_map[row.neuron_class]], axis = 1).astype(float)
    ...
    a.custom['custom_features_by_neuron_class']["depol_mean"] = a.custom['custom_features_by_neuron_class'].apply(lambda row: a.repo.simulations.df.etl.q(simulation_id=row.simulation_id).iloc[0].simulation.__dict__['config']["Stimulus_" + a.analysis_config.custom['depol_bc_key'] + '_' + map_to_use[row.neuron_class]]['MeanPercent'], axis = 1).astype(float)
    a.custom['custom_features_by_neuron_class']["depol_sd"] = a.custom['custom_features_by_neuron_class'].apply(lambda row: a.repo.simulations.df.etl.q(simulation_id=row.simulation_id).iloc[0].simulation.__dict__['config']["Stimulus_" + a.analysis_config.custom['depol_bc_key'] + '_' + map_to_use[row.neuron_class]]['SDPercent'], axis = 1).astype(float)
    ...
    a.custom['custom_features_by_neuron_class']["true_mean_conductance"] = a.custom['custom_features_by_neuron_class'].apply(lambda row: input_conductance_by_neuron_class_df.etl.q(neuron_class=row.neuron_class)['resting_conductance'].iloc[0] * row['depol_mean'] / 100.0, axis = 1).astype(float)
    ######################################################################
